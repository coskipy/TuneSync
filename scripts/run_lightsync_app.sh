...
LOG_FILE="/tmp/lightsync_app.log"
...
nohup "$PY" -u -m lightsync_app >"$LOG_FILE" 2>&1 &
new_pid=$!
...
if ! kill -0 "$new_pid" 2>/dev/null; then
  echo "ERROR: LightSync failed to start (pid=$new_pid)." >&2
  if [[ -f "$LOG_FILE" ]]; then
...
echo "LightSync started (pid=$new_pid). Logs: $LOG_FILE"
